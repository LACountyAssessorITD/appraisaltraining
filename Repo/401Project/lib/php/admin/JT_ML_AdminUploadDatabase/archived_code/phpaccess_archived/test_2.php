...
<body>
	<table>
		<?php
			...
			$AR_to_CH_mdb  = "SELECT t.CertNo, t.FiscalYear, t.CertType, t.Status, t.EarnedHours, t.RequiredHours,
								t.CurrentYearBalance, t.PriorYearBalance, t.CarryToYear1, t.CarryToYear2, t.CarryToYear3,
								t.CarryForwardTotal";
			$AR_to_CH_mdb .= "  FROM AnnualReq t;";

			$result = $mdb->query($AR_to_CH_mdb)->fetchAll(\PDO::FETCH_ASSOC);
			// print_r($result);

			$source = $mdb->query($AR_to_CH_mdb);



			$AR_to_CH_srvr = "INSERT INTO New_CertHistory (CertNo, CertYear, CertType, Status, HoursEarned, RequiredHours,
															CurrentYearBalance, PriorYearBalance, CarryToYear1,
															CarryToYear2, CarryToYear3, CarryForwardTotal)";
			$AR_to_CH_srvr .= " VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
			// $AR_to_CH_srvr .= ""
			...
			while ($row = $source->fetch()) {
				...
				$params = array($CertNo, $CertYear, $CertType, $Status, $HoursEarned, $RequiredHours,
								...
								$CarryForwardTotal);
				$stmt = sqlsrv_query( $conn, $AR_to_CH_srvr, $params);
				if( $stmt == false ) {
					echo "<br />";
					print_r( "heyyyyyyyyy " + $CertNo );
					die( print_r( sqlsrv_errors(), true));
				}
